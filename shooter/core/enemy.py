...
class BossEnemy(BaseEnemy):
    ...
    def __init__(self, x, y):
        ...
        w, h = img.get_size()
        scaled = pygame.transform.scale(img, (int(w * 1.8), int(h * 1.8)))
        self.image = scaled
...
def spawn_boss(walls):
    """Spawn the boss in the centre-right of the map."""
    boss = BossEnemy(WIDTH * 3 // 4, HEIGHT // 2)
    return pygame.sprite.GroupSingle(boss)